    ...
    Base.extend(ns, {
        ...
        excludeEnv:function(snippetAst,params){
        	for (obj in snippetAst){
        		if ((typeof (snippetAst[obj]) == 'object' || Array.isArray(snippetAst[obj])) && snippetAst[obj])
        ...
        convertShaderToSnippedAst : function(code){
            var fullAst;
            if(typeof(code) == "function"){
                code = "METHOD=" + code.toString();
                fullAst = this.getSanitizedAst(code, {}).body[0].expression.right;
            ...
            for (var i = 0; i < duplicatedParams.length; i++) {
            	tmpParams[duplicatedParams[i]] = duplicatedParams[i];
            }
            params = [];
            ...
            fullAst["params"]=[];
            for (i in params){
            	fullAst["params"].push({"name":params[i],"type":"Identifier"})
            }
            return fullAst;
        },
