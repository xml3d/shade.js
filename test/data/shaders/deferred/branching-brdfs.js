...
void main ( void ) {
    if(_env_factor < 0.25) {
        ...
        gl_FragColor[1] = vec4(0.0, modelViewMatrixN * vec3(1, 0, 0));
        gl_FragColor[2] = vec4(vec3(1, 1, 0), 0.5);
        return;
    } else {
        if(_env_factor > 0.75) {
            gl_FragColor[0] = vec4(0, ( modelViewMatrix * vec4(_env_position, 1.0) ).xyz);
            gl_FragColor[1] = vec4(0.0, modelViewMatrixN * vec3(0, 0, 1));
            gl_FragColor[2] = vec4(vec3(1, 0, 1), 0.5);
            return;
        } else {
            gl_FragColor[0] = vec4(1, ( modelViewMatrix * vec4(_env_position, 1.0) ).xyz);
            gl_FragColor[1] = vec4(0.0, vec3(1, 0, 0.5));
